...
@Composable
fun UserItem(user: UserResponse, isExpanded: Boolean, onToggle: () -> Unit) {
    AnimatedContent(
        targetState = isExpanded,
        transitionSpec = {
            (fadeIn() + scaleIn()).togetherWith(fadeOut() + scaleOut())
        },
        label = "User Expand Animation"
    ) { expanded ->
        Card(
            modifier = Modifier
                .fillMaxWidth()
            ...
            if (expanded) {
                UserItemExpanded(user, onToggle)
            } else {
                UserItemCollapsed(user, onToggle)
            }
        }
    }
}
